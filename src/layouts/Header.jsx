...
export default function Navbar() {
  ...
  if (loading) {
    return (
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/90 backdrop-blur-sm">
        {/* Skeleton loading state can be added here */}
      </nav>
    );
  }
      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center py-3 px-6">
              ...
              <div className="relative">
                ...
                >
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={user.profileImage ? `${API_URL}${user.profileImage}` : ''} alt={getUserName()} />
        ...
        <div className="md:hidden">
          <button 
            ...
            aria-label="Toggle menu"
          >
            {isOpen ? <FaTimes size={20} className="text-gray-600" /> : <FaBars size={20} className="text-gray-600" />}
          </button>
        </div>
      </div>
          ...
          <div className="px-6 py-4 space-y-2">
            {navItems.map((item) => (
              <Link key={item.name} to={item.path} onClick={() => setIsOpen(false)}
                className={`flex items-center gap-3 py-3 px-4 rounded-lg transition-all ${
                  location.pathname === item.path ? "bg-green-100 text-green-700" : "text-gray-600 hover:bg-gray-50"
                }`}
              >
                <span className="text-base">{item.icon}</span>
                {item.name}
            ...
            {isAuthenticated ? (
              <div className="space-y-2">
                ...
                {/* Mobile profile section with Avatar */}
                <div className="flex items-center gap-3 py-3 px-4 bg-gray-50 rounded-lg">
                  <Avatar className="h-8 w-8">
                    <AvatarImage src={user.profileImage ? `${API_URL}${user.profileImage}` : ''} alt={getUserName()} />
                    <AvatarFallback className="bg-gradient-to-br from-green-500 to-emerald-600 text-white font-semibold text-sm">
