...
// import { postToEsewa } from '../../utils/esewa'; // 2. Import your AuthContext (adjust path if needed)
...
//   // 3. Get user data from the AuthContext
//   const { user, isAuthenticated } = useContext(AuthContext);

//   // The current plan is now derived from the logged-in user's data
//   const currentUserPlan = user?.subscription || 'Basic'; // Assumes user object has a 'subscription' property
...
//     try {
//       // 4. FIX: Use the correct key 'token' to get the item
...
//         // 5. FIX: Use Vite's env variable syntax and your base URL
...
//           return (
//             <Card key={index} className={`flex flex-col ${plan.isPopular ? 'border-green-500 border-2 shadow-lg' : ''}`}>
//               {plan.isPopular && (
...
//                   // The button is disabled if not logged in, if it's the current plan, a free plan, or currently loading
//                   disabled={!isAuthenticated || isCurrent || plan.price === 'Free' || isLoading}
//                 >
//                   {isLoading 
//                     ? "Processing..."
//                     : isCurrent 
