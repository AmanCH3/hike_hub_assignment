...
import { Mountain, MapPin, Clock, Star, Zap, Users, Eye } from "lucide-react";
import { useAdminTrail } from "../../hooks/admin/useAdminTrail";
import { Link } from "react-router-dom";
import { ViewTrailDialog } from "../admin/trail_management/ViewTrailDailog";
import { JoinTrailDialog } from "../trails/JoinTrailDialog"; // ✅ 1. IMPORT THE NEW DIALOG
import { useState } from "react";
...
const FeaturedTrails = () => {
  ...
  // ✅ 2. ADD STATE FOR THE JOIN DIALOG
  ...
  // This function is no longer needed as onOpenChange handles it
  // const handleCloseDialog = () => { ... };

  // Fetch only 3 trails for the featured section for better performance
  const { trails, isLoading, isError, error } = useAdminTrail({ limit: 3 });
    ...
    <section className="py-10 px-4 bg-gradient-to-br from-green-50 to-blue-50">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12">
          <div>
            <h2 className="text-4xl font-bold text-gray-900 mb-2">Featured Trails</h2>
            <p className="text-xl text-gray-600">Discover breathtaking hiking trails</p>
          </div>
          <Link to="/trails" className="inline-flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-900 border-2 border-gray-200 hover:border-gray-300 px-6 py-3 rounded-full font-semibold transition-all duration-200 shadow-sm hover:shadow-md text-sm">
            View All Trails
            ...
            <div key={trail._id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group">
              <div className="relative overflow-hidden h-64">
                ...
                {trail.averageRating > 0 && (
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm rounded-full px-2 py-1 flex items-center gap-1">
                    <Star className="h-4 w-4 text-yellow-400 fill-current" />
                    <span className="text-sm font-semibold text-gray-900">{trail.averageRating.toFixed(1)}</span>
                    <span className="text-xs text-gray-600">({trail.numRatings})</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-6">
                <div className="mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <Mountain className="h-5 w-5 text-green-600" />
                    <h3 className="text-xl font-bold text-gray-900 group-hover:text-green-600 transition-colors">{trail.name}</h3>
                  </div>
                  <div className="flex items-center text-gray-600 mb-2">
                    <MapPin className="h-4 w-4 mr-1" />
                    <span className="text-sm">{trail.location}</span>
                ...
                {trail.description && (<p className="text-gray-600 text-sm mb-4 line-clamp-2">{trail.description}</p>)}
                <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="flex items-center gap-2">
                        <Mountain className="h-4 w-4 text-gray-500" />
                        <div><p className="text-xs text-gray-500">Elevation</p><p className="text-sm font-semibold">{trail.elevation}m</p></div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-gray-500" />
                        <div><p className="text-xs text-gray-500">Duration</p><p className="text-sm font-semibold">{trail.duration.min}-{trail.duration.max}h</p></div>
                    </div>
                    {trail.distance && (<div className="flex items-center gap-2"><Zap className="h-4 w-4 text-gray-500" /><div><p className="text-xs text-gray-500">Distance</p><p className="text-sm font-semibold">{trail.distance}km</p></div></div>)}
                    {trail.numRatings > 0 && (<div className="flex items-center gap-2"><Users className="h-4 w-4 text-gray-500" /><div><p className="text-xs text-gray-500">Reviews</p><p className="text-sm font-semibold">{trail.numRatings}</p></div></div>)}
                </div>
                <div className="mt-6 flex gap-2">
                  {/* ✅ 3. UPDATE BUTTONS */}
                  <button 
                    onClick={() => setTrailToJoin(trail)}
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-lg text-sm font-medium text-center transition-colors flex items-center justify-center gap-2"
                  >
                    Join This Hike
                  </button>
                  <button 
                    onClick={() => handleViewTrail(trail)}
                    className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-3 rounded-lg text-sm font-medium text-center transition-colors flex items-center justify-center gap-2"
                  >
                    <Eye className="h-4 w-4" />
                    Quick View
                  </button>
